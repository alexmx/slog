...
struct DurationParserTests {
    ...
    func throwsForEmpty() {
        #expect(throws: ValidationError.self) {
            _ = try parseDuration("", optionName: "--timeout")
        }
    }
    ...
    func throwsForInvalidFormat() {
        #expect(throws: ValidationError.self) {
            _ = try parseDuration("abc", optionName: "--timeout")
        }
    }
    ...
    func throwsForNegative() {
        #expect(throws: ValidationError.self) {
            _ = try parseDuration("-5s", optionName: "--timeout")
        }
    }
    ...
    func throwsForZero() {
        #expect(throws: ValidationError.self) {
            _ = try parseDuration("0s", optionName: "--timeout")
        }
    }
